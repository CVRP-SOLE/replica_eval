...
class OpenSceneMask3D(nn.Module):
    def __init__(
        self,
        config,
        hidden_dim,
        ...
        self.semantic_proj = nn.Linear(768, self.mask_dim)
        self.clip_proj = nn.Linear(self.backbone.PLANES[7]+768, self.mask_dim)
        self.mask_features_head = conv(
            self.backbone.PLANES[7]+768,
            self.mask_dim,
            kernel_size=1,
            stride=1,
            bias=True,
        ...
        self.class_embed_head = nn.Linear(hidden_dim, 768)

        import pickle, math
        with open("text/CLIP_ViT-L-14_replica.pickle", "rb") as f:
            self.text_embed = pickle.load(f)
        self.bg_feature = nn.Parameter(torch.Tensor(1, 768))
        ...
        for _ in range(num_shared):
            ...
            for i, hlevel in enumerate(self.hlevels):
                ...
                tmp_squeeze_attention.append(
                    nn.Linear(sizes[hlevel]+768, self.mask_dim)
                )
    ...
    def forward(
        self, x, data_openscene=None, point2segment=None, raw_coordinates=None, clip_feat=None, guide=None, caption=None, caption_entity=None, is_eval=False
    ):  
        with torch.no_grad():
            """
            openseg_features_pool = [f.mean(dim=0) for f in clip_feat]
            openseg_features_pool = torch.stack(openseg_features_pool, dim=0)
            rd = torch.einsum('b d, c d -> b c d', openseg_features_pool, self.text_embed)
            t = self.text_embed.unsqueeze(0).repeat(len(clip_feat), 1, 1)
            text_guide = torch.cat([rd, t], dim=-1)

            guide_rd = []
            for f, g in zip(openseg_features_pool, guide):
                rd = torch.einsum('d, c d -> c d', f, g)
                guide = torch.cat([rd, g], dim=-1)
                guide_rd.append(guide)
            """
            self.openscene = mink_unet(in_channels=3, out_channels=768, D=3, arch='MinkUNet18A').cuda()
            self.openscene.load_state_dict(torch.load("/hdd/yyzhao/Mask3D_caption_entity_scannet200_pretrained/openscene/checkpoint.pt"))

            pcd_features_clip = self.openscene(data_openscene)[0].decomposed_features
            pcd_features_clip_F = [
                torch.nn.functional.normalize(pcd_features_clip[i], dim=-1) for i in range(len(pcd_features_clip))
            ]

            batch_coords, batch_feats = ME.utils.sparse_collate(x.decomposed_coordinates, pcd_features_clip_F)
            ...
            if self.training:
                max_size = 0
                for c, e in zip(caption, caption_entity):
                    # max_size = c.shape[0] + e["key"].shape[0] if c.shape[0] + e["key"].shape[0] > max_size else max_size
                    max_size = c.shape[0] if c.shape[0] > max_size else max_size
                ...
                for c, e in zip(caption, caption_entity):
                    ...
                    # sample 6 classes
                    """
                    idx = np.random.choice(np.arange(18), size=6, replace=True)
                    random_class_embed = self.text_embed[idx]
                    """

                    # key.append(torch.cat([c, e["key"]], dim=0))
                    key.append(c)
                    semantic_key_mask.append(midx)
                    semantic_key_idx.append(idx)

                batched_key = torch.stack(
                    [
                        key[k][semantic_key_idx[k], :]
                        for k in range(len(caption))
                    ]
                )
                batched_semantic_key_mask = torch.stack(semantic_key_mask)
                batched_semantic_key_mask = batched_semantic_key_mask[..., None].repeat(1, 1, self.num_queries)
            else:
                batched_key = self.text_embed.unsqueeze(0).repeat(len(caption), 1, 1)

        batched_key = self.semantic_proj(batched_key)
        ...
        predictions_class = []
        predictions_open_class = []
        predictions_open_caption_class = []
        predictions_open_caption_entity_class = []
        predictions_mask = []
        for decoder_counter in range(self.num_decoders):
            ...
            for i, hlevel in enumerate(self.hlevels):
                if self.train_on_segments:
                    output_class, open_cost, open_caption_cost, open_caption_entity_cost, outputs_mask, attn_mask = self.mask_module(
                        queries,
                        mask_features,
                        mask_segments,
                        len(aux) - hlevel - 1,
                ...
                if self.training:
                    output = self.cross_semantic_attention[decoder_counter][i](
                        output,
                        batched_key.permute((1, 0, 2)),
                        memory_mask = batched_semantic_key_mask.repeat_interleave(
                            self.num_heads, dim=0
                        ).permute((0, 2, 1)),
                        query_pos=query_pos,
                    )
                ...
                predictions_open_class.append(open_cost)
                predictions_open_caption_class.append(open_caption_cost)
                predictions_open_caption_entity_class.append(open_caption_entity_cost)
                predictions_mask.append(outputs_mask)

        if self.train_on_segments:
            output_class, open_cost, open_caption_cost, open_caption_entity_cost, outputs_mask = self.mask_module(
                queries,
                mask_features,
                mask_segments,
                0,
        ...
        return {
            ...
            "pred_open_logits": predictions_open_class[-1],
            "pred_open_caption_logits": predictions_open_caption_class[-1],
            "pred_open_caption_entity_logits": predictions_open_caption_entity_class[-1],
    ...
    def mask_module(
        ...
        open_cost = []
        for o, g in zip(outputs_class, guide):
            class_cost = o @ g.t() * 100
            bg_cost = o @ self.bg_feature.t() * 100
            open_cost.append(torch.cat([class_cost, bg_cost], dim=-1))

        """
        open_caption_cost = []
        for o, c in zip(outputs_class, caption):
            class_cost = o @ c.t() * 100
            bg_cost = o @ self.bg_feature.t() * 100
            open_caption_cost.append(torch.cat([class_cost, bg_cost], dim=-1))

        open_caption_entity_cost = []
        for o, e in zip(outputs_class, caption_entity):
            class_cost = o @ e["guide"].t() * 100
            bg_cost = o @ self.bg_feature.t() * 100
            open_caption_entity_cost.append(torch.cat([class_cost, bg_cost], dim=-1))
        """

        open_caption_cost = []
        for o, c in zip(outputs_class, guide):
            class_cost = o @ c.t() * 100
            bg_cost = o @ self.bg_feature.t() * 100
            open_caption_cost.append(torch.cat([class_cost, bg_cost], dim=-1))

        open_caption_entity_cost = []
        for o, e in zip(outputs_class, guide):
            class_cost = o @ e.t() * 100
            bg_cost = o @ self.bg_feature.t() * 100
            open_caption_entity_cost.append(torch.cat([class_cost, bg_cost], dim=-1))

        class_cost = outputs_class @ self.text_embed.t() * 100
        bg_cost = outputs_class @ self.bg_feature.t() * 100
        cost = torch.cat([class_cost, bg_cost], dim=-1)
        ...
        if ret_attn_mask:
            ...
            """
            mask = attn_mask.decomposed_features
            mask = [m.detach() > 0.5 for m in mask]
            """
            ...
            if point2segment is not None:
                return cost, open_cost, open_caption_cost, open_caption_entity_cost, output_segments, attn_mask
            else:
                return (
                    cost,
                    open_cost,
        ...
        if point2segment is not None:
            return cost, open_cost, open_caption_cost, open_caption_entity_cost, output_segments
        else:
            return cost, open_cost, open_caption_cost, open_caption_entity_cost, outputs_mask.decomposed_features
    ...
    def _set_aux_loss(
        ...
        outputs_class, 
        open_outputs_class, 
        open_caption_outputs_class, 
        open_caption_entity_outputs_class,
        outputs_seg_masks
    ):
        ...
        return [
            {
                "pred_logits": a, 
                "pred_open_logits": b, 
                "pred_open_caption_logits": c, 
                "pred_open_caption_entity_logits": d,
                "pred_masks": e
                }
            for a, b, c, d, e in zip(
                outputs_class[:-1], 
                open_outputs_class[:-1],
                open_caption_outputs_class[:-1],
                open_caption_entity_outputs_class[:-1],
                outputs_seg_masks[:-1]
            )
        ]
