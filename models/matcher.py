...
class HungarianMatcher(nn.Module):
    ...
    def memory_efficient_forward(self, outputs, targets, mask_type):
        ...
        for b in range(bs):
            ...
            out_open_prob = outputs["pred_open_logits"][b].softmax(
                -1
            )[:, :-1]
            out_open_caption_prob = outputs[ "pred_open_caption_logits"][b].softmax(
                -1
            )[:, :-1]
            out_open_caption_entity_prob = outputs["pred_open_caption_entity_logits"][b].softmax(
                -1
            )[:, :-1]
            out_open_ensemble_prob = (out_open_prob + out_open_caption_prob + out_open_caption_entity_prob) / 3.
            ...
            filter_ignore_idx = (cost_class == -2.0)
            cost_class[filter_ignore_idx] = -out_open_ensemble_prob[filter_ignore_idx]
            """
            cost_class = -out_open_ensemble_prob
