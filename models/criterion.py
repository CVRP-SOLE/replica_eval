...
class SetCriterion(nn.Module):
    ...
    def loss_labels_nel(self, outputs, targets, indices, num_masks, mask_type):
        ...
        for logit_key in ["pred_open_logits", "pred_open_caption_logits", "pred_open_caption_entity_logits"]:
            for i, logit in enumerate(outputs[logit_key]):
                ...
                loss_masks.append(sigmoid_focal_loss(src_logit[0], target_one_hot[0], num_masks))
                loss_dices.append(dice_loss_jit(src_logit[0], target_one_hot[0], num_masks))
        ...
        target_classes[idx] = target_classes_o
        target_one_hot = F.one_hot(target_classes, num_classes=-1)[:, :, :19].float()
        num_masks = target_classes.shape[1]
        batch_size = target_classes.shape[0]

        for b in range(batch_size):
            loss_masks.append(sigmoid_focal_loss(src_logits[b], target_one_hot[b], num_masks))
            loss_dices.append(dice_loss_jit(src_logits[b], target_one_hot[b], num_masks))
        """
