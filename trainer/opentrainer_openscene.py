...
class InstanceSegmentation(pl.LightningModule):
    def __init__(self, config):
        ...
        """
        import pickle
        with open("/mnt/data/seungjun/code/Mask3D/text/CLIP-ViT-L-14_scannet200.pickle", "rb") as f:
            self.model.text_embed = pickle.load(f)
        """
        ...
        weight_dict = {
            "loss_nel_focal": 20.,
            "loss_nel_dice": matcher.cost_dice,
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            for i, f in enumerate(file_names):
                feat = torch.load("/hdd/yyzhao/mla_replica_evaluate/replica_multiview_openseg_full/" + f[:-4] + "_0.pt").cuda()
                feat = feat[unique_maps[i]]
                feat = torch.nn.functional.normalize(feat, dim=-1, eps=1e-5).float()
                openseg_features.append(feat)

            guide_feat_collate = []
            for o, t in zip(openseg_features, target):
                gt_mask = t['masks']
                guide_feat = []
                for g in gt_mask:
                    guide_feat.append(o[g].mean(dim=0).unsqueeze(0))
                guide_feat_collate.append(torch.cat(guide_feat, dim=0))
            
            """
            caption_feat_collate = []
            for f in file_names:
                with open("/mnt/data/seungjun/cache/scannet200_captions/" + f + ".pickle", "rb") as f:
                    caption = pickle.load(f)
                caption_feat_collate.append(torch.cat(caption["features"], dim=0))

            caption_feat_entity_collate = []
            for f in file_names:
                with open("/mnt/data/seungjun/cache/scannet200_nounphrases_coco_preprocessed/" + f + ".pickle", "rb") as f:
                    caption_entity = pickle.load(f)
                caption_feat_entity_collate.append(caption_entity)
            """

        # FOR INFERENCE #
        caption_feat_collate = guide_feat_collate
        caption_feat_entity_collate = guide_feat_collate
        # ============= #
        ...
        try:
            output = self.forward(
                ...
                raw_coordinates=raw_coordinates,
                clip_feat = openseg_features,
                guide=guide_feat_collate,
                caption=caption_feat_collate,
                caption_entity=caption_feat_entity_collate,
            )
    ...
    def eval_step(self, batch, batch_idx):
        ...
        data_openscene = ME.SparseTensor(
            coordinates=data.coordinates,
            features=torch.ones_like(data.features).cuda(),
            device=self.device,
        )
    ...
    def eval_instance_step(
        ...
        prediction[self.decoder_id][
            "pred_logits"
        ] = prediction[self.decoder_id]["pred_logits"].sigmoid()[..., :-1]
        """
    ...
    def eval_instance_epoch_end(self):
        ...
        gt_data_path = f"/hdd/yyzhao/mla_replica_evaluate/data/processed/replica/instance_gt"
