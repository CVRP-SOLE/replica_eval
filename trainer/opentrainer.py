...
class InstanceSegmentation(pl.LightningModule):
    def __init__(self, config):
        ...
        self.clip_feat_path = config.general.openseg_path
    ...
    def eval_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            for i, f in enumerate(file_names):
                feat = torch.load(os.path.join(self.clip_feat_path, f[:-4] + "_0.pt")).to(self.device)
        ...
        try:
            output = self.forward(
                ...
                guide=guide_feat_collate, # TRICK FOR INFERENCE, has no meaning #
                caption=guide_feat_collate, # TRICK FOR INFERENCE, has no meaning #
                caption_entity=guide_feat_collate, # TRICK FOR INFERENCE, has no meaning #
                is_eval=True,
            )
    ...
    def eval_instance_step(
        ...
        clip_feat = torch.load(os.path.join(self.clip_feat_path, file_names[0][:-4] + "_0.pt")).to(self.device)
        clip_feat = clip_feat[inv[0]]
    ...
    def eval_instance_epoch_end(self):
        ...
        gt_data_path = f"data/processed/replica/instance_gt"
